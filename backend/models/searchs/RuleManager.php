<?php

namespace backend\models\searchs;

use Yii;
use yii\base\Model;
use yii\data\ArrayDataProvider;
use backend\models\RuleManager as ModelRuleManager;
use backend\components\rules\RouteRule;

/**
 * Description of RuleManager
 */
class RuleManager extends Model
{
    /**
     * @var string name of the rule
     */
    public $name;

    public function rules()
    {
        return [
            [['name'], 'safe']
        ];
    }

    /**
     * Search RuleManager
     * @param array $params
     * @return \yii\data\ActiveDataProvider|\yii\data\ArrayDataProvider
     */
    public function search($params)
    {
        /* @var \yii\rbac\Manager $authManager */
        $authManager = Yii::$app->authManager;
        $models = [];
        $included = !($this->load($params) && $this->validate() && trim($this->name) !== '');
        foreach ($authManager->getRules() as $name => $item) {
            if ($name != RouteRule::RULE_NAME && ($included || stripos($item->name, $this->name) !== false)) {
                $models[$name] = new ModelRuleManager($item);
            }
        }

        return new ArrayDataProvider([
            'allModels' => $models,
			'sort' => ['attributes' => ['name', 'className', 'createdAt', 'updatedAt']],
        ]);
    }
}
